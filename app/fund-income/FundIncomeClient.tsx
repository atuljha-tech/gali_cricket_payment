// app/fund-income/FundIncomeClient.tsx
'use client'
...
import {
  ...
  Wallet, ShoppingBag, TrendingUp, Calendar, User,
  MoreHorizontal, X, Check, AlertTriangle, Award,
  Users, HandCoins, Trophy
} from 'lucide-react'

// ── Types ────────────────────────────────────────────────────────────────────
type IncomeCategory = 'donation' | 'sponsorship' | 'subscription' | 'tournament' | 'merchandise' | 'other'
...
// ── Category meta ────────────────────────────────────────────────────────────
const INCOME_CATEGORIES: Record<IncomeCategory, { label: string; icon: React.ReactNode; color: string; bg: string }> = {
  donation:      { label: 'Donation',    icon: <HandCoins size={14} />, color: 'text-green-400',   bg: 'bg-green-500/15 border-green-500/25' },
  sponsorship:   { label: 'Sponsorship', icon: <Award size={14} />,     color: 'text-purple-400',  bg: 'bg-purple-500/15 border-purple-500/25' },
  subscription:  { label: 'Subscription',icon: <Users size={14} />,     color: 'text-blue-400',    bg: 'bg-blue-500/15 border-blue-500/25' },
  tournament:    { label: 'Tournament',  icon: <Trophy size={14} />,    color: 'text-yellow-400',  bg: 'bg-yellow-500/15 border-yellow-500/25' },
  merchandise:   { label: 'Merchandise', icon: <ShoppingBag size={14} />, color: 'text-orange-400', bg: 'bg-orange-500/15 border-orange-500/25' },
...
function IncomeModal({
  ...
  async function submit() {
    if (!form.amount || !form.reason || !form.date) { 
    ...
    try {
      const url    = initial ? `/api/income/${initial._id}` : '/api/income'
      ...
      <div className="w-full max-w-lg bg-slate-800 border border-slate-700/60 rounded-2xl shadow-2xl flex flex-col max-h-[90vh]"
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-700/50 flex-shrink-0">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-bold text-white text-sm">{initial ? 'Edit Income' : 'Record Income'}</h2>
          </div>
        ...
        <div className="px-6 py-5 space-y-4 overflow-y-auto flex-1">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input type="number" min="1" placeholder="0"
                  className="bg-slate-900/80 border border-slate-600/60 text-slate-100 placeholder-slate-500 rounded-xl pl-8 pr-4 py-2.5 w-full focus:outline-none focus:border-green-500 focus:ring-2 focus:ring-green-500/20 transition-all text-sm"
            ...
            <div className="space-y-1.5">
              ...
              <input type="date"
                className="bg-slate-900/80 border border-slate-600/60 text-slate-100 rounded-xl px-4 py-2.5 w-full focus:outline-none focus:border-green-500 focus:ring-2 focus:ring-green-500/20 transition-all text-sm"
          ...
          <div className="space-y-1.5">
            ...
            <input type="text" placeholder="e.g. Monthly subscription fees" maxLength={100}
              className="bg-slate-900/80 border border-slate-600/60 text-slate-100 placeholder-slate-500 rounded-xl px-4 py-2.5 w-full focus:outline-none focus:border-green-500 focus:ring-2 focus:ring-green-500/20 transition-all text-sm"
              value={form.reason} onChange={e => set('reason', e.target.value)} />
          </div>

          {/* Source */}
          <div className="space-y-1.5">
            <label className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Source (optional)</label>
            <input type="text" placeholder="e.g. John Doe, ABC Corp, Team A" maxLength={100}
              className="bg-slate-900/80 border border-slate-600/60 text-slate-100 placeholder-slate-500 rounded-xl px-4 py-2.5 w-full focus:outline-none focus:border-green-500 focus:ring-2 focus:ring-green-500/20 transition-all text-sm"
          ...
          <div className="space-y-1.5">
            ...
            <div className="grid grid-cols-3 gap-2">
              {(Object.keys(INCOME_CATEGORIES) as IncomeCategory[]).map(cat => (
                <button key={cat} type="button"
                  onClick={() => set('category', cat)}
                  className={`flex items-center gap-1.5 px-3 py-2 rounded-xl border text-xs font-medium transition-all ${
                    form.category === cat
                      ? `${INCOME_CATEGORIES[cat].bg} ${INCOME_CATEGORIES[cat].color} ring-1 ring-current/30`
                      : 'bg-slate-900/60 border-slate-700/40 text-slate-500 hover:border-slate-500/60 hover:text-slate-300'
                  }`}>
                  {INCOME_CATEGORIES[cat].icon}
                  {INCOME_CATEGORIES[cat].label}
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <textarea rows={3} placeholder="Describe where the money came from..." maxLength={500}
              className="bg-slate-900/80 border border-slate-600/60 text-slate-100 placeholder-slate-500 rounded-xl px-4 py-2.5 w-full focus:outline-none focus:border-green-500 focus:ring-2 focus:ring-green-500/20 transition-all text-sm resize-none"
        ...
        <div className="flex gap-3 px-6 pb-5 pt-3 border-t border-slate-700/40 flex-shrink-0">
          ...
          <button onClick={submit} disabled={loading}
            className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-green-600 hover:bg-green-500 disabled:opacity-50 text-white rounded-xl text-sm font-semibold transition-all shadow-lg shadow-green-900/30">
    ...
    <header className="bg-slate-900/95 border-b border-slate-700/50 backdrop-blur-xl sticky top-0 z-20">
      <div className="max-w-5xl mx-auto px-4 py-3.5 flex items-center gap-3">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p className="text-sm font-bold text-white">Fund Income</p>
            <p className="text-[10px] text-slate-500">Gali Online Cricket · Receipts</p>
          </div>
        </div>
      </div>
    </header>
...
function IncomeCard({
  ...
  isAdmin: boolean
  onEdit: (t: IncomeTransaction) => void
  onDelete: (id: string) => void
}) {
  const cat = INCOME_CATEGORIES[txn.category] ?? INCOME_CATEGORIES.other
    ...
    <div className="bg-slate-800/50 border border-slate-700/40 rounded-2xl p-4 md:p-5 hover:border-slate-600/50 transition-all">
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-start gap-3 flex-1 min-w-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-3 text-[11px] text-slate-500 mb-2 flex-wrap">
              ...
              {txn.source && (
                <span className="flex items-center gap-1"><HandCoins size={11} />{txn.source}</span>
              )}
            </div>
            {txn.details && (
        ...
        <div className="flex flex-col items-end gap-2 flex-shrink-0">
          <p className="text-lg font-black text-green-400">+₹{txn.amount.toLocaleString('en-IN')}</p>
          {isAdmin && (
...
// ── Main export ──────────────────────────────────────────────────────────────
export default function FundIncomeClient() {
  ...
  const [editTxn,      setEditTxn]      = useState<IncomeTransaction | undefined>()
  const [activeFilter, setActiveFilter] = useState<IncomeCategory | 'all'>('all')

  const fetchData = useCallback(async (p = 1) => {
    setLoading(true)
    try {
      const res  = await fetch(`/api/income?page=${p}&limit=20`)
  ...
  const breakdown = (Object.keys(INCOME_CATEGORIES) as IncomeCategory[]).map(cat => ({
    cat,
  ...
  const Content = (
    <div className="min-h-screen">
      ...
      <div className="relative overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-green-950/80 via-slate-900 to-slate-950" />
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              <div className="flex items-center gap-2 mb-3">
                <span className="inline-flex items-center gap-1.5 text-xs font-semibold text-green-400 bg-green-500/15 border border-green-500/25 px-3 py-1 rounded-full">
                  <TrendingUp size={11} /> Fund Income
                </span>
              </div>
              <h1 className="text-4xl md:text-5xl font-black text-white tracking-tight">
                GOC <span className="text-green-400">Income</span>
              </h1>
              <p className="text-slate-400 mt-2 text-base max-w-md">
                Transparent record of all funds received by the team.
              </p>
              <div className="flex items-center gap-4 mt-4">
                <div className="flex items-center gap-2 px-4 py-2.5 bg-green-500/10 border border-green-500/20 rounded-xl">
                  <TrendingUp size={16} className="text-green-400" />
                  <div>
                    <p className="text-[10px] text-slate-500 font-medium uppercase tracking-wider">Total Received</p>
                    <p className="text-xl font-black text-green-400">₹{totalIncome.toLocaleString('en-IN')}</p>
                  </div>
                </div>
            ...
            {isAdmin && (
              <button onClick={() => { setEditTxn(undefined); setShowModal(true) }}
                className="flex items-center gap-2.5 px-6 py-3.5 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-500 hover:to-emerald-500 text-white font-bold rounded-xl shadow-xl shadow-green-900/30 transition-all duration-200 active:scale-95 text-sm self-start md:self-auto">
      ...
      <div className="max-w-5xl mx-auto px-4 md:px-8 pb-12">
        ...
        {transactions.length > 0 && (
          <div className="flex items-center gap-2 flex-wrap mb-5">
            ...
            {(Object.keys(INCOME_CATEGORIES) as IncomeCategory[]).filter(cat => transactions.some(t => t.category === cat)).map(cat => (
              <button key={cat} onClick={() => setActiveFilter(activeFilter === cat ? 'all' : cat)}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold border transition-all ${
                  activeFilter === cat
                    ? `${INCOME_CATEGORIES[cat].bg} ${INCOME_CATEGORIES[cat].color}`
                    : 'border-slate-700/50 text-slate-400 hover:border-slate-500 hover:text-slate-300'
                }`}>
                {INCOME_CATEGORIES[cat].icon} {INCOME_CATEGORIES[cat].label}
              </button>
            ))}
          </div>
        )}
        ...
        {loading && transactions.length === 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-28 text-center">
            ...
            <h3 className="text-lg font-bold text-white mb-1">
              {activeFilter === 'all' ? 'No income recorded yet' : `No ${INCOME_CATEGORIES[activeFilter].label} income`}
            </h3>
            <p className="text-slate-500 text-sm max-w-xs">
              {isAdmin && activeFilter === 'all'
                ? 'Start recording income so everyone can see where the funds come from.'
                : 'Try selecting a different category filter.'}
            </p>
            {isAdmin && activeFilter === 'all' && (
              <button onClick={() => { setEditTxn(undefined); setShowModal(true) }}
                className="mt-6 flex items-center gap-2 px-5 py-2.5 bg-green-600 hover:bg-green-500 text-white font-bold rounded-xl transition-all active:scale-95 text-sm">
          ...
          <div className="space-y-3">
            {filtered.map(txn => (
              <IncomeCard key={txn._id} txn={txn} isAdmin={isAdmin}
        ...
        {!loading && transactions.length > 0 && (
          <p className="text-center text-xs text-slate-600 mt-6">
            {transactions.length} income record{transactions.length !== 1 ? 's' : ''} · Total ₹{totalIncome.toLocaleString('en-IN')} received · GOC Fund
          </p>
        )}
      </div>
    </div>
