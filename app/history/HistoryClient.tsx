...
import { CheckCircle2, Clock, FileText, IndianRupee, TrendingUp, Trash2, Loader2 } from 'lucide-react'
import { MONTH_NAMES } from '@/lib/fineCalculator'

interface Payment {
  _id: string; month: number; year: number
  amount: number; fine: number; total: number; paidAmount?: number
...
export default function HistoryClient({ adminName, adminEmail }: { adminName: string; adminEmail: string }) {
  ...
  const collected  = payments.filter(p => p.status === 'paid').reduce((s, p) => s + (p.paidAmount ?? p.total), 0)
      ...
      <div className="space-y-5">
        ...
        <div className="card p-4">
          <div className="flex flex-wrap items-center gap-3">
            <select className="bg-slate-900/80 border border-slate-600/60 text-slate-100 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-green-500"
              value={month} onChange={e => { setMonth(Number(e.target.value)); setPage(1) }}>
        ...
        <div className="card overflow-hidden">
          {loading ? (
            ...
            <>
              {/* Desktop header — no fine column */}
              <div className="hidden md:grid md:grid-cols-[2fr_140px_90px_120px_130px_80px] gap-4 px-5 py-3 border-b border-slate-700/40 bg-slate-800/40">
                {['Player', 'Period', 'Amount', 'Status', 'Marked By', ''].map(h => (
              ...
              <div className="divide-y divide-slate-700/30">
                {payments.map(p => (
                  <div key={p._id} className="group hover:bg-slate-700/20 transition-colors">
                    ...
                    <div className="md:hidden px-4 py-4 space-y-2">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="text-right">
                          <p className="text-base font-bold text-white">₹{p.paidAmount ?? p.total}</p>
                          {p.status === 'paid'
                            ? <span className="badge-paid"><CheckCircle2 size={10} />Paid</span>
                            : <span className="badge-pending"><Clock size={10} />{p.status === 'partial' ? 'Partial' : 'Pending'}</span>}
                        </div>
                      </div>
                      <div className="flex items-center justify-between">
                        {p.receiptNo ? (
                          <div className="flex items-center gap-3">
                            ...
                            <Link href={`/receipt/${p._id}`} className="text-xs text-green-400 hover:text-green-300">Receipt →</Link>
                          </div>
                        ) : <span />}
                        <button onClick={() => setConfirmId(p._id)}
                          className="flex items-center gap-1 text-xs px-2.5 py-1.5 border border-slate-600/60 text-slate-500 hover:border-red-500/60 hover:text-red-400 rounded-lg">
                    ...
                    {/* Desktop */}
                    <div className="hidden md:grid md:grid-cols-[2fr_140px_90px_120px_130px_80px] gap-4 px-5 py-3.5 items-center">
                      <div>
                        <p className="text-sm font-semibold text-white">{p.playerId?.name || '—'}</p>
                        <p className="text-xs text-slate-500">{p.playerId?.phone}</p>
                      </div>
                      <p className="text-sm text-slate-300">{MONTH_NAMES[p.month]} {p.year}</p>
                      <p className="text-sm font-bold text-white">₹{p.paidAmount ?? p.total}</p>
                      <div>
                        {p.status === 'paid'
                          ? <span className="badge-paid"><CheckCircle2 size={10} />Paid</span>
                          : <span className="badge-pending"><Clock size={10} />{p.status === 'partial' ? 'Partial' : 'Pending'}</span>}
                      </div>
                      <p className="text-xs text-slate-400">{(p.adminId as any)?.name || '—'}</p>
                      <div className="flex items-center gap-2">
                        {p.receiptNo ? (
                          <Link href={`/receipt/${p._id}`} className="text-green-400 hover:text-green-300" title={p.receiptNo}>
                            <FileText size={15} />
                          </Link>
                        ) : <span className="w-4" />}
                        <button onClick={() => setConfirmId(p._id)}
                          className="opacity-0 group-hover:opacity-100 p-1 text-slate-600 hover:text-red-400 rounded">
                          <Trash2 size={14} />
                        </button>
                      </div>
                    </div>
        ...
        {pages > 1 && (
          <div className="flex items-center justify-center gap-3">
            <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
              className="px-5 py-2.5 text-sm border border-slate-600/50 rounded-xl disabled:opacity-40 hover:border-slate-500 text-slate-300">Previous</button>
            <span className="text-sm text-slate-500 bg-slate-800 border border-slate-700 px-4 py-2 rounded-xl">{page} / {pages}</span>
            <button onClick={() => setPage(p => Math.min(pages, p + 1))} disabled={page === pages}
              className="px-5 py-2.5 text-sm border border-slate-600/50 rounded-xl disabled:opacity-40 hover:border-slate-500 text-slate-300">Next</button>
          </div>
        )}
      </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-400 mt-1">
                  This will remove the payment for{' '}
                  <span className="text-white font-semibold">{confirmPayment.playerId?.name}</span>
                  {' '}({MONTH_NAMES[confirmPayment.month]} {confirmPayment.year} · ₹{confirmPayment.paidAmount ?? confirmPayment.total}).
                </p>
              </div>
            </div>
            <div className="flex gap-3">
              <button onClick={() => setConfirmId(null)}
                className="flex-1 py-2.5 border border-slate-600/50 text-slate-300 hover:text-white hover:bg-slate-700/60 rounded-xl text-sm font-medium">Cancel</button>
              <button onClick={handleDelete} disabled={deleting}
                className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-red-600 hover:bg-red-500 disabled:opacity-50 text-white rounded-xl text-sm font-semibold">
