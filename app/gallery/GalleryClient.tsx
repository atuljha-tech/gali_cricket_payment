...
function UploadModal({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) {
  ...
  function processFiles(files: FileList | File[]) {
    ...
    const tooLarge = arr.filter(f => f.size > 5 * 1024 * 1024)
    if (tooLarge.length) setError(`${tooLarge.length} file(s) skipped — each must be under 5MB`)
    else setError('')
  ...
  async function handleUpload() {
    ...
    setProgress(0)
    let uploaded = 0
    const name = uploaderName || 'Anonymous'
    ...
    for (const item of previews) {
      try {
        const res = await fetch('/api/gallery', {
          ...
          body: JSON.stringify({ imageData: item.dataUrl, uploaderName: name }),
        })
        if (!res.ok) {
          const d = await res.json()
          setError(d.error || 'Some uploads failed')
        }
      } catch {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-md">
      ...
      >
        ...
        <div className="px-6 py-5 space-y-4 overflow-y-auto flex-1">
          {/* Drop zone */}
          <div
            className={`relative border-2 border-dashed rounded-2xl transition-all duration-300 cursor-pointer p-8 ${
          ...
          >
            ...
            <div className="flex flex-col items-center gap-3 text-center">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 mt-1">Select up to 20 photos · JPG, PNG, WEBP · Max 5MB each</p>
              </div>
              {previews.length > 0 && (
                <p className="text-xs text-green-400 font-semibold">Click again to add more photos</p>
              )}
            </div>
          </div>
...
// Public version of gallery header (without admin sidebar)
    ...
    <div className="min-h-screen">
      ...
      <div className="relative overflow-hidden">
        ...
        {[...Array(6)].map((_, i) => (
          <div
            key={i}
            className="absolute w-1 h-1 bg-yellow-400/30 rounded-full animate-pulse-slow"
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              ...
              <p className="text-slate-400 mt-2 text-base max-w-md">
                Captured moments from the pitch. Every match, every celebration, every memory — preserved forever.
              </p>
        ...
        {loading && photos.length === 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-32 text-center">
            ...
            <p className="text-slate-500 text-sm max-w-xs mb-8">
              Be the first to share a moment from GOC. Upload your photo and start the gallery!
            </p>
            <button
              onClick={() => setShowUpload(true)}
              className="flex items-center gap-2 px-6 py-3 bg-yellow-500 hover:bg-yellow-400 text-slate-900 font-bold rounded-xl transition-all duration-200 active:scale-95"
            >
              <Upload size={17} />
              Share First Memory
            </button>
          ...
          <>
            <div className="gallery-grid">
              {photos.map((photo) => (
                <div
                  key={photo._id}
                  className="relative group aspect-square overflow-hidden rounded-xl bg-slate-800/60 border border-slate-700/30 hover:border-slate-500/50 transition-all duration-300 cursor-pointer"
                ...
                >
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col justify-between p-3">
                    {/* Top row — delete button (admin only) */}
            ...
            {page < pages && (
              <div className="flex justify-center mt-10">
                <button
                  onClick={loadMore}
                  disabled={loading}
                  className="flex items-center gap-2 px-8 py-3 border border-slate-600/50 hover:border-slate-500 text-slate-300 hover:text-white rounded-xl text-sm font-medium transition-all duration-200 disabled:opacity-50"
                >
            ...
            <p className="text-center text-xs text-slate-600 mt-8">
              {photos.length} of {total} photos · GOC Memories Gallery
            </p>
          </>
        )}
      </div>
