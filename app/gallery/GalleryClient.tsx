...
// ── Main ──────────────────────────────────────────────────────────────────────
export default function GalleryClient({ initialData }: { initialData?: { photos: Photo[], total: number, pages: number } }) {
  ...
  const fetchPhotos = useCallback(async (p = 1) => {
    if (p === 1 && !initialData) setLoading(true); else setLoadingMore(true)
    try {
  ...
  // Automatic background fetching for remaining pages
  useEffect(() => {
