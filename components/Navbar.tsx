...
export default function Navbar({ adminName, adminEmail }: { adminName?: string; adminEmail?: string }) {
  ...
  // Sidebar nav links — rendered identically in desktop sidebar and mobile drawer
  const navLinks = (
    <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
      ...
      <div className="pt-3">
        ...
        <Link
          href="/qr"
          onClick={() => setMobileOpen(false)}
          className={pathname === '/qr' ? 'nav-link-active' : 'nav-link'}
        >
    ...
    <>
      ...
      {mobileOpen && (
        <>
          ...
          />
          {/* Drawer panel */}
          <div className="md:hidden fixed left-0 top-14 bottom-0 z-30 w-72 bg-slate-900/98 border-r border-slate-700/50 overflow-y-auto flex flex-col">
            {navLinks}
            {adminFooter}
          </div>
        </>
